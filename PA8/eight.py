...
def tiles_out_of_row_column(puzzle):
    ...
    total = 0
    #ROWS
    row1 = puzzle.state[0:2]
    row2 = puzzle.state[3:5]
    row3 = puzzle.state[6:8]

    #now we need to check individual numbers
    if 1 not in row1:
        total += 1
    ...
    #COLUMNS
    indices = [0,3,6]
    column1 = [puzzle.state[x] for x in indices]
    indices = [1,4,7]
    column2 = [puzzle.state[x] for x in indices]
    indices = [2,5,8]
    column3 = [puzzle.state[x] for x in indices]

    #individual number check again
    if 1 not in column2:
        total += 1
...
def manhattan_distance_to_goal(puzzle):
    ...
    #now have to iterate through all numbers
    for index, value in enumerate(puzzle.state):
        if (index != value):
            manhattan_total += (abs(index_2d_equivalents[index][0] - index_2d_equivalents[value][0]) + (abs(index_2d_equivalents[index][1] - index_2d_equivalents[value][1])) ) 
            


    return manhattan_total #change this!
